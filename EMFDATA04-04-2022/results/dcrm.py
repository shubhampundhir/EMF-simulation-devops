...
def duration (in_arys):
    out_array = [];
    for arr in in_arys:
        x_end = np.max(np.nonzero(arr))
        x_start = np.min(np.nonzero(arr))
        no_samp_perd = x_end - x_start
        duration = no_samp_perd * samp_time  ## duration of pulse in one file
        out_array = np.append(out_array,duration);  # True if any values of array are non-zero else false for zero/empty array
    return out_array
...
in_curr_chnl=[I1,I2,I3,T1,T2,T3,DI1,DI2,DI3,DI4,DI5,DI6] # It is assumed that if current channels (DI) are inactive then DR1,DR2,DR3,DR4,DR5,DR6 will also be inactive
act_chnl=[]
eval_arys=[];
act_chnl = idntf_act_chnl(in_curr_chnl);# 1 for active channels and 0 for non-active channels

in_chnls=[I1,I2,I3,T1,T2,T3,DI1,DI2,DI3,DI4,DI5,DI6,DR1,DR2,DR3,DR4,DR5,DR6]
in_chnls_dur=duration (in_chnls)
print (in_chnls_dur)
