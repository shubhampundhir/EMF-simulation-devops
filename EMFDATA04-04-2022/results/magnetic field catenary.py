...
eradi = eradi*db/2;
print (eradi*2);
Im=[442.2,442.2,442.2,442.2,442.2,442.2,0,0]; # Voltage array
#Im=[100*math.sqrt(3)]; # Voltage array
Pm=[240,0,120,0,240,120,0,0]; # Phase angle array
...
for a in range(0,nphase,1):
    ...
    #I.append((Im[a]/math.sqrt(3))*complex(x,y));
...
for j in range(0,div+1,1):
    ...
    for jj in range(0,nphase,1):
        for jjj in range(0,cond_div,1):
            ...
            #print (Re);
            #print(Rb);
            Bx=Bx+((I[jj])/(D*D)*((La)/(Rb)+(1-La)/(Re))*((Zp-Zs)*b-(Yp-Ys)*c)*10e-8);# converted to tesla
