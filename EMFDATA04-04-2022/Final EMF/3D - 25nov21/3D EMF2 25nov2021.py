...
def seg_matrix(pp):
        ...
        for ttt in range(0, (sec[pp]-1), 1):#will run for all sec
            ...
            if tt==ttt:
              ...
              Idelta4 = math.sqrt((forth) * (forth) + (eradi) * (eradi));
                ...
                delta1 = math.sqrt((XF - XB) * (XF - XB) + (YF - YB) * (YF - YB) + (ZF - ZB) * (ZF - ZB));
                delta2 = math.sqrt((XF-XE)*(XF-XE)+(YF-YE)*(YF-YE)+(ZF-ZE)*(ZF-ZE));
                Idelta1 = math.sqrt((XF - XB) * (XF - XB) + (YF + YB) * (YF + YB) + (ZF - ZB) * (ZF - ZB));
                Idelta2 = math.sqrt((XF - XE) * (XF - XE) + (YF + YE) * (YF + YE) + (ZF - ZE) * (ZF - ZE));
                delta3 = math.sqrt((XS - XB) * (XS - XB) + (YS - YB) * (YS - YB) + (ZS - ZB) * (ZS - ZB));
                delta4 = math.sqrt((XS - XE) * (XS - XE) + (YS - YE) * (YS - YE) + (ZS - ZE) * (ZS - ZE));
                Idelta3 = math.sqrt((XS - XB) * (XS - XB) + (YS + YB) * (YS + YB) + (ZS - ZB) * (ZS - ZB));
                Idelta4 = math.sqrt((XS - XE) * (XS - XE) + (YS + YE) * (YS + YE) + (ZS - ZE) * (ZS - ZE));
    ...
    #print(DQ1);
    DQ=numpy.matmul(DQ1,DQ3);#DQ is equal to DQ/4*pi*E
...
for gg in range(0, (div+1), 1): #FOR EACH MEASUREMENT point
    ...
    for www in range(0, nphase, 1):
        ...
        for tttt in range(0, sec[www]-1, 1):
            ...
            E1_MAIN=(((Q[tttt]-(DQ[tttt]*(D1*D1-D2*D2))/2*(L*L))*(B*C))-D);# Main conductor orthogonal component

            A1 = (((D3*D3-D4*D4)-L*L)/(2*L));
            B1 = math.sqrt(D4*D4-A1*A1);
            C1 = (2*L*(D3+D4))/(D3*D4*(D3+D4-L)*(D3+D4+L));
            D11= (((-DQ[tttt])*B1)/L)*(1/D3-1/D4);
            E1_IMAG = ((((-Q[tttt])-((-DQ[tttt])*(D3*D3-D4*D4))/2*(L*L))*(B1*C1)-D11)); # Imaginery conductor orthogonal component

            E=(DQ[tttt]/L)*math.log((D1+D2+L)/(D1+D2-L),math.e);
            F=(DQ[tttt]*(D1+D2))/(2*D1*D2);
            E1_DESS_MAIN=((Q[tttt]*(1/D2-1/D1))+ E-F);# Main conductor parallel component

            E1 = ((-DQ[tttt])/L)*math.log((D3+D4+L)/(D3+D4-L),math.e)
            F1 = ((-DQ[tttt])*(D3+D4))/(2*D3*D4);
            E1_DESS_IMAG = (((-Q[tttt])*(1/D4-1/D3))+ E1-F1);# Imaginery conductor parallel component

            E_XMAIN=E1_MAIN*(XB-((A/L)*(XE-XB))-XT)/B;# ortho
            E_XIMAG = E1_IMAG*(XB-((A/L)*(XE-XB))-XT)/B1;# ortho

            E_DESS_XMAIN=E1_DESS_MAIN*((XE-XB)/L);# parallel
            E_DESS_XIMAG = E1_DESS_IMAG*((XE-XB)/L);

            EX_MAIN = EX_MAIN+(E_XMAIN+E_DESS_XMAIN);
            EX_IMAG = EX_IMAG+(E_XIMAG+E_DESS_XIMAG);

            E_YMAIN = E1_MAIN*(YB-((A/L)*(YE-YB))-YT)/B;
            E_YIMAG = E1_IMAG*(YB -((A/L)*(-YE+YB))-YT)/B1;


            E_DESS_YMAIN = E1_DESS_MAIN*((YE-YB)/L);
            E_DESS_YIMAG = E1_DESS_IMAG*((-YE+YB)/L);

            EY_MAIN = EY_MAIN+(E_YMAIN+E_DESS_YMAIN);
            EY_IMAG = EY_IMAG+(E_YIMAG+E_DESS_YIMAG);

            E_ZMAIN = E1_MAIN*(ZB-((A/L)*(ZE-ZB))-ZT)/B;
            E_ZIMAG = E1_IMAG*(ZB-((A/L)*(ZE-ZB))-ZT)/B1;

            E_DESS_ZMAIN = E1_DESS_MAIN*((ZE-ZB)/L);
            E_DESS_ZIMAG = E1_DESS_IMAG*((ZE-ZB)/L);
